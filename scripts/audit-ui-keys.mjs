...
for (const loc of locales) {
  ...
  // Filter out data.* tour content strings
  const nonDataMissing = legKeys.filter(k => !k.startsWith('data.') && !newKeys.includes(k));
  console.log(`[Locale: ${loc}] Non-data missing UI keys: ${nonDataMissing.length}`);
  if (nonDataMissing.length > 0) {
    console.log('   Missing UI keys:', nonDataMissing);
  }
}
