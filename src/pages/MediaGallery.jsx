...
const MediaGallery = () => {
  ...
  const { galleryImages, loading, error } = useMedia();
  ...
  return (
    <main className="min-h-screen bg-[#070D19] px-6 pb-24 pt-[140px] text-ivory-50">
      ...
      <section className="container mx-auto max-w-7xl">
        ...
        {galleryImages.length === 0 ? <div className="rounded-2xl border border-white/10 bg-white/5 p-12 text-center text-ivory-300">{t("mediaGallery.empty", "No published media is available yet.")}</div> : <div className="grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">{galleryImages.map((asset, index) => <button type="button" key={asset.id} onClick={() => setSelectedIndex(index)} className="group overflow-hidden rounded-2xl border border-white/10 bg-[#0E1726] text-start focus:outline-none focus:ring-2 focus:ring-gold-500"><img src={asset.url} alt={asset.label} loading="lazy" className="h-64 w-full object-cover transition-transform duration-300 group-hover:scale-105" /><span className="block truncate px-4 py-3 text-sm text-white">{asset.label}</span></button>)}</div>}
      </section>
